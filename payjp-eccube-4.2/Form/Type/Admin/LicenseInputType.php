    ...
    public function buildForm(FormBuilderInterface $builder, array $optinos)
    {
        $builder
            ->add('license_key', TextType::class, [
                ...
                'constraints' => [
                    new Assert\NotBlank(array(
                            'message' => trans('pay_jp.admin.license.license_key.error.blank')
                        )
                    ),
                    new Assert\Regex(array(
                            'pattern' => '/^[\w-]*$/',
                            'match' => true,
                            'message' => trans('pay_jp.admin.license.license_key.error.regex')
                        )
                    )
                ],  
            ])
            ->add('email', TextType::class, [
                ...
                'constraints' => [
                    new Assert\NotBlank(array(
                            'message' => trans('pay_jp.admin.license.email.error.blank')
                        )
                    ),
                    new Assert\Regex(array(
                            'pattern' => '/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,})$/i',
                            'match' => true,
                            'message' => trans('pay_jp.admin.license.email.error.regex')
                        )
                    )
                ],  
            ]);
