...
class ProductClassExController extends ProductClassController{
    ...
    public function registerProduct($id){
        ...
        $res = $this->stripe_service->registerProduct($Product);
    ...
    public function registerProductClass($prod_class, $interval){
        ...
        // $stripe_service = $this->container->get('plg_stripe_rec.service.stripe_service');        
        return $this->stripe_service->registerPrice($prod_class, $interval);
    }    
    public function updateProductClass($prod_class){
        ...
        $new_pc = $this->stripe_service->updatePrice($prod_class);
        if(empty($new_pc)){
        ...
        foreach($rec_orders as $rec_order){
            ...
            $res = $this->stripe_service->updateSubscription($rec_order->getSubscriptionId(), $new_pc->getStripePriceId(), $price_id);
            if(empty($res)){
    ...
    private function updateSchedule($old_price_id, $old_pc, $new_class, $schedule){
        ...
        // echo $schedule_id . "<br>";
        // $stripe_service = $this->container->get('plg_stripe_rec.service.stripe_service');        
        $stripe_schedule = $this->stripe_service->retrieveNotStartingSchedule($schedule_id);
        ...
        if($stripe_schedule){
            ...
            $resp = $this->stripe_service->updateSchedule($schedule_id, [
                'phases'    =>  $new_phases,
