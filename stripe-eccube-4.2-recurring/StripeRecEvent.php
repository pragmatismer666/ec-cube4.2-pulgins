...
class StripeRecEvent implements EventSubscriberInterface
{
    ...
    protected $stripe_recurring_util_service;
    ...
    )
    {
        ...
        $this->config_service = $this->container->get('plg_stripe_rec.service.admin.plugin.config');
        $this->pb_service = $this->container->get('plg_stripe_rec.service.pointbundle_service');
        $this->coupon_service = $this->container->get('plg_stripe_rec.service.coupon_service');    
        $this->stripe_recurring_util_service = $this->container->get("plg_stripe_recurring.service.util");
    ...
    public function onCartPage(TemplateEvent $event) {
        ...
        $is_multi_product = !$this->config_service->get(ConfigService::MULTI_PRODUCT);
    ...
    public function onAdminOrderEdit(TemplateEvent $event) 
    {
        ...
        if ($Order->isInitialRec()) {
            ...
            $bundles = $this->pb_service->getBundleProducts($rec_order);
            
            $price_sum = $this->pb_service->getPriceSum($rec_order);        
            extract($price_sum);
            ...
            if($coupon_str = $rec_order->getCouponDiscountStr()){
                $initial_discount = $this->coupon_service->couponDiscountFromStr($initial_amount, $coupon_str);
                $recurring_discount = $this->coupon_service->couponDiscountFromStr($recurring_amount, $coupon_str);
            }else{
                $initial_discount = 0;
    ...
    public function onProdEditComplete(EventArgs $args){
        ...
        $res = $this->stripe_recurring_util_service->saveDefaultClass($def_pc, $register_flg, $price_change_flg);
        if(!$res){
            $form->get('register_flg')->addError(new FormError($this->stripe_recurring_util_service->getErrMsg()));
        }
    }
    ...
    public function onShoppingConfirmTwig(TemplateEvent $event)
    {
        ...
        if ($Order->getPayment()->getMethodClass() === StripeRecurringNagMethod::class
            ...
            $purchase_point = $this->pb_service->calculatePurchasePoint($purchase_point, $after_days);
            ...
            $bundles = $this->pb_service->getBundleProductsOrderByShipping($Order, $bundle_include_arr);
    ...
    public function onShoppingIndexTwig(TemplateEvent $event){
        ...
        if($Order) {
            ...
            if ($Order->getPayment()->getMethodClass() === StripeRecurringNagMethod::class
                ...
                $bundles = $this->pb_service->getBundleProductsOrderByShipping($Order, null);
