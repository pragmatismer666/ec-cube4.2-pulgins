...
class StripeRecurringNagMethod implements PaymentMethodInterface
{
    ...
    public function checkout()
    {
        ...
        $bundles = $this->pb_service->getBundleProducts($this->Order, $bundle_include_arr);        
        if(empty($bundles)){
            if(empty($subscription_items_initial_pre)){
                $phases = [
                    [
        ...
        if($coupon_enable){
            ...
            if(!empty($coupon_id)){
                ...
                $coupon_data = $this->coupon_service->retrieveCoupon($coupon_id);
                if(empty($coupon_data)){
                    $result->setSuccess(false);
                    $result->setErrors([ $this->coupon_service->getError() ]);
                    return $result;
                }
                $coupon_discount = $this->coupon_service->couponDiscountAmount($initial_price, $coupon_data);
        ...
        if(!empty($coupon_id)){
            ...
            $stripeOrder->setCouponDiscountStr($this->coupon_service->couponDiscountStr($coupon_data));
    ...
    private function multi_checkout($rec_config)
    {
        ...
        foreach($order_items as $order_item) {
            ...
            if (!empty($coupon_data)) {
                $coupon_discount = $this->coupon_service->couponDiscountAmount($initial_price, $coupon_data);
                if ($coupon_discount === false) {
                    $errors[] = $this->coupon_service->getError();
                    continue;
                }
                foreach($schedule_params['phases'] as $k => $v) {
                    $schedule_params['phases'][$k]['coupon'] = $coupon_id;
            ...
            for ($i = 0; $i < $quantity; $i++) {
                ...
                $this->dispatcher->dispatch($args, StripeRecEvent::REC_ORDER_CREATED);
            }
        }
        ...
        $this->dispatcher->dispatch(
            ...
            StripeRecEvent::RECURRING_CHECKOUT_FINALIZE
        );
    ...
    public function paydayOptionProcess($schedule_params, $initial_price, $temp_product_id, $interval,$currency='jpy'){
        ...
        $payday_options = $this->config_service->getPaymentDateOptions();
    ...
    public function isProrateOption($purchase_point, $interval){
        $option_1 = $this->config_service->get(ConfigService::PAYDAY_OPTION_1);
        $option_2 = $this->config_service->get(ConfigService::PAYDAY_OPTION_2);
        $payment_date = $this->config_service->get(ConfigService::PAYMENT_DATE);
    ...
    private function getShippingProductId(){
        ...
        if(empty($stripeShippingProduct) || !$stripe_service->validateShippingProductExist($stripeShippingProduct->getStripeShippingProdId())){
            ...
            $name='送料';
            $description='送料';
            $res = $this->stripe_service->registerShippingProduct($name,$description,$stripeShippingProduct);
            $stripeShippingProduct = $this->entityManager->getRepository(StripeRecShippingProduct::class)->get();
        }
